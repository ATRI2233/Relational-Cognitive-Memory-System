...
- **话题切换惩罚删除** — `_update_momentum()` 移除 `depth *= 0.5` 话题切换阻力。惯性更新 `depth * 0.70` 已做自然衰减，该惩罚叠床架屋
- **自曝标记补全** — `_DEPTH_MARKERS['self_disclosure']` 新增 `想`/`不想`/`没法`/`只能`/`不敢`，扩大 self_disclosure 检测覆盖面

### Fixed

- **depth 断崖下跌** — turn 13→14 的 depth 从 0.05（旧）恢复至 0.11（新），同一话题连续段 depth 稳定在 0.08~0.13

### Testing

- 30 轮 LLM 对话压力测试通过：depth 峰值 0.13（↑），均值 0.05（↑ 2.5×），0 错误
- 全部 64 项存量测试无回归

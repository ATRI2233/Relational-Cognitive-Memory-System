import json
import logging
from datetime import datetime

logger = logging.getLogger("rcms")


class ContextMixin:
    """Narrative Context / Prompt Compressor — 供 AstrBot 注入 / standalone chat 使用"""

    def _session_warmup(self, user_id: str, session_id: str, turn_count: int) -> str:
        """新 session 预热：读取上一个 session 的 dangling + 最近蒸馏摘要"""
        if turn_count > 1 or not user_id or not session_id:
            return ""
        row = self.conn.execute("""
            SELECT dangling_threads, last_active
            FROM session_state
            WHERE session_id != ? AND last_active IS NOT NULL
            ORDER BY last_active DESC LIMIT 1
        """, (session_id,)).fetchone()
        if not row:
            return ""
        dangling, last_active = row
        if not dangling:
            return ""
        p = self._load_prompts().get("narrative_context", {})
        dangling_label = p.get("dangling_label", "未完成")
        parts = []
        if dangling:
            try:
                dt = json.loads(dangling)
                if isinstance(dt, dict) and dt.get("threads"):
                    parts.append(dangling_label + "：" + "、".join(dt["threads"][:3]))
            except (json.JSONDecodeError, ValueError):
                pass
        if last_active:
            try:
                days = (datetime.now() - datetime.fromisoformat(str(last_active))).days
                if days == 0:
                    parts.append(p.get("warmup_days_today", "距上次对话：今天"))
                elif days == 1:
                    parts.append(p.get("warmup_days_yesterday", "距上次对话：昨天"))
                else:
                    parts.append(p.get("warmup_days_ago", "距上次对话：{days} 天前").format(days=days))
            except (ValueError, TypeError):
                pass
        if not parts:
            return ""
        return "\n".join(f"  · {part}" for part in parts)

    def narrative_context(self, stance: str, session_id: str | None = None,
                           memories: list | None = None, long_term: dict | None = None,
                           user_input: str = "", user_id: str = "") -> str:
        p = self._load_prompts()
        nc = p.get("narrative_context", {})
        channel_labels = p.get("channel_labels", {
            "recent": "时间·重要性", "resonance": "语义检索", "skeleton": "图谱关联",
        })
        display_order = p.get("memories_display_order", ["resonance", "skeleton", "recent"])

        parts = []

        # ── 三通道记忆 ──
        if memories:
            grouped = {}
            for content, tag in memories:
                grouped.setdefault(tag, []).append(content)
            ch_lines = []
            for key in display_order:
                items = grouped.get(key)
                if items:
                    label = channel_labels.get(key, key)
                    ch_lines.append(f"【{label}】\n" + "\n".join(f"  · {c}" for c in items))
            if ch_lines:
                parts.append(nc.get("memories_title", "相关记忆") + ":\n" + "\n\n".join(ch_lines))

        # ── 会话统计 ──
        turn_count = 0
        dangling = ""
        if session_id:
            try:
                row = self.conn.execute(
                    "SELECT turn_count, dangling_threads "
                    "FROM session_state WHERE session_id = ?", (session_id,)
                ).fetchone()
                if row:
                    turn_count = row[0] or 0
                    dangling = row[1] or ""
            except Exception:
                logger.exception(f"RCMS: 读取 session_state 失败 session={session_id}")

        # ── 新 session 预热 ──
        warmup = self._session_warmup(user_id, session_id, turn_count)
        if warmup:
            parts.append(nc.get("session_warmup_title", "[上次聊到]") + "\n" + warmup)

        # ── 轮数 ──
        if turn_count:
            parts.append(nc.get("turn_count_template", "聊了 {count} 轮").format(count=turn_count))

        # ── 用户画像: traits + quirks ──
        remaining_traits_tpl = nc.get("remaining_traits_template", "及其他 {count} 条特质")
        quirk_label = nc.get("quirk_prefix", "小细节小习惯")
        profile_lines = []
        if long_term:
            trait_details = long_term.get('trait_details', [])
            trait_details.sort(key=lambda x: x.get("strength", 0), reverse=True)
            all_traits = [td for td in trait_details if not td["text"].startswith("[小细节小习惯]")]
            max_show = 5
            for td in all_traits[:max_show]:
                strength = td.get("strength", 0)
                prefix = "" if strength >= 5 else "↘ " if strength <= 2 else "· "
                profile_lines.append(f"{prefix}{td['text']}")
            remaining = len(all_traits) - max_show
            if remaining > 0:
                profile_lines.append(remaining_traits_tpl.format(count=remaining))
            quirks = [(td.get("strength", 0), td["text"].replace("[小细节小习惯] ", ""))
                      for td in trait_details if td["text"].startswith("[小细节小习惯]")]
            quirks.sort(key=lambda x: x[0], reverse=True)
            if quirks:
                q_mark = "↘ " if any(q[0] <= 2 for q in quirks) else ""
                profile_lines.append(f"{q_mark}{quirk_label}: {'、'.join(q[1] for q in quirks[:2])}")
        if profile_lines:
            parts.append(nc.get("profile_title", "他是什么样的") + ":\n" + '\n'.join(f'  · {t}' for t in profile_lines))

        # ── 结构化画像 ──
        struct_lines = []
        if long_term:
            prefs = long_term.get('preferences', {})
            if prefs.get('likes'):
                struct_lines.append(nc.get("pref_likes", "喜好") + ": " + '、'.join(prefs['likes'][:5]))
            if prefs.get('dislikes'):
                struct_lines.append(nc.get("pref_dislikes", "不喜欢") + ": " + '、'.join(prefs['dislikes'][:3]))
            si = long_term.get('self_identity', [])
            if si:
                struct_lines.append(nc.get("self_identity", "自我认同") + ": " + '、'.join(si[:3]))
            bounds = long_term.get('boundaries', [])
            if bounds:
                struct_lines.append(nc.get("boundaries", "雷区") + ": " + '、'.join(bounds[:3]))
        if struct_lines:
            parts.append(nc.get("struct_title", "结构化画像") + ":\n" + '\n'.join(f'  · {s}' for s in struct_lines))

        # ── 共同语境 ──
        context_title = nc.get("context_title", "共同语境")
        joke_label = nc.get("joke_label", "梗")
        entity_label_tpl = nc.get("entity_label_template", "他提过的{type}")
        ctx_lines = []
        if long_term:
            shared = long_term.get('shared_contexts', [])
            jokes = [s.replace('[梗] ', '') for s in shared if s.startswith('[梗]')][:2]
            other = [s for s in shared if not s.startswith('[梗]')][:2]
            ctx_lines.extend(f"{joke_label}: {j}" for j in jokes)
            ctx_lines.extend(other)

            entities = long_term.get('entities', [])
            if entities:
                grouped_ents = {}
                seen_names = set()
                for e in entities:
                    name = e.get('name', '')
                    if not name or name in seen_names:
                        continue
                    seen_names.add(name)
                    etype = e.get('type', 'auto')
                    if etype not in grouped_ents:
                        grouped_ents[etype] = []
                    tag = f" ({e.get('relation', '')})" if e.get('relation') else ""
                    grouped_ents[etype].append(f"{name}{tag}")
                type_labels = {'person': '人', 'place': '地方', 'concept': '概念', 'activity': '活动', 'auto': '相关'}
                for etype, items in grouped_ents.items():
                    label = type_labels.get(etype, etype)
                    ctx_lines.append(entity_label_tpl.format(type=label) + ": " + '、'.join(items[:4]))
        if ctx_lines:
            parts.append(context_title + ":\n" + '\n'.join(f'  · {c}' for c in ctx_lines))

        # ── 图谱关系链 ──
        graph_chain_title = nc.get("graph_chain_title", "【图谱关系链】")
        graph_paths = getattr(self, '_graph_paths', [])
        if graph_paths:
            parts.append(graph_chain_title + "\n" + '\n'.join(f'  · {p}' for p in graph_paths))

        # ── 未完成话题 ──
        stale_prefix = nc.get("dangling_stale_prefix", "↘ ")
        if dangling:
            try:
                dt_data = json.loads(dangling)
                if isinstance(dt_data, dict):
                    dt_list = dt_data.get("threads", [])
                    since_turn = dt_data.get("turn", 0)
                    if dt_list and turn_count - since_turn <= 10:
                        stale = turn_count - since_turn > 5
                        prefix = stale_prefix if stale else ""
                        dangling_display = prefix + "、".join(dt_list[:3])
                        parts.append(nc.get("dangling_label", "未完成") + ": " + dangling_display)
                elif isinstance(dt_data, list) and dt_data:
                    parts.append(nc.get("dangling_label", "未完成") + ": " + '、'.join(dt_data[:3]))
            except (json.JSONDecodeError, ValueError):
                pass

        parts.append(nc.get("footer", "→ 以上是你通过长期对话积累的对他的了解，用来更好地理解他的意图。"))

        return nc.get("prefix", "[RCMS 关系上下文,这里面放置了你对他的了解,按需使用]") + "\n" + "\n\n".join(parts)

    async def prompt_compressor(self, user_id: str, session_id: str, user_input: str,
                           memories: list | None = None,
                           long_term: dict | None = None) -> str:
        p = self._load_prompts()
        pc = p.get("prompt_compressor", {})
        channel_labels = p.get("channel_labels", {
            "recent": "时间·重要性", "resonance": "语义检索", "skeleton": "图谱关联",
        })

        if memories is None:
            memories = await self.retrieve_memories(user_id, user_input, 'engaged', session_id=session_id)
        grouped = {}
        order = []
        for content, tag in memories:
            if tag not in grouped:
                grouped[tag] = []
                order.append(tag)
            grouped[tag].append(content)
        mem_lines = []
        for key in order:
            if key not in grouped:
                continue
            label = channel_labels.get(key, key)
            items = grouped[key][:2]
            mem_lines.append(f"【{label}】\n" + "\n".join(f"  · {c}" for c in items))
        mem_block = "\n\n".join(mem_lines) if mem_lines else ""

        # 图谱关系链
        graph_paths = getattr(self, '_graph_paths', [])
        gp_block = ""
        graph_chain_title = pc.get("graph_chain_title", "【图谱关系链】")
        if graph_paths:
            gp_block = "\n" + graph_chain_title + "\n" + "\n".join(f"  · {pth}" for pth in graph_paths)

        lt_block = ""
        if long_term:
            shared_ctx = long_term.get('shared_contexts', [])
            if shared_ctx:
                ctx = '、'.join(shared_ctx[:3])
                lt_block += "\n" + pc.get("shared_context_title", "【共同语境】") + ctx
            traits = long_term.get('identity_traits', [])
            if traits:
                trait_strs = [t for t in traits if not t.startswith('[小细节小习惯]')][:3]
                if trait_strs:
                    lt_block += "\n" + pc.get("trait_title", "【用户特质】") + '；'.join(trait_strs)
                quirks = [t for t in traits if t.startswith('[小细节小习惯]')][:2]
                if quirks:
                    lt_block += "\n" + pc.get("quirk_title", "【小细节小习惯】") + '；'.join(q.replace('[小细节小习惯] ', '') for q in quirks)
            si = long_term.get('self_identity', [])
            if si:
                lt_block += "\n" + pc.get("identity_title", "【自我认同】") + '、'.join(si[:2])
            bounds = long_term.get('boundaries', [])
            if bounds:
                lt_block += "\n" + pc.get("boundary_title", "【雷区】") + '、'.join(bounds[:2])

        prompt = pc.get("mood_title", "【当前心理状态】") + "\n" + pc.get("mood_default", "自然地聊")
        if mem_block:
            prompt += "\n\n" + pc.get("memory_title", "【相关记忆】") + "\n" + mem_block
        if gp_block:
            prompt += gp_block
        if lt_block:
            prompt += lt_block
        prompt += "\n\n" + pc.get("bottom_line_label", "【底线】") + "\n" + pc.get("bottom_line_text", "不主动说教。不假装完全理解。疲惫时简短但不冷漠。") + "\n\n" + pc.get("user_template", "用户: {user_input}\n你:").format(user_input=user_input)
        return prompt
